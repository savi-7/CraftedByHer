...
router.get("/customer-hub-orders", async (req, res) => {
  try {
    ...
    // Get orders at customer hubs (ready for pickup/delivery) - EXCLUDE delivered orders
    ...
    const orders = await Order.find({
      $and: [
        ...
        {
          orderStatus: { $ne: 'delivered' }  // Explicitly exclude delivered orders
        },
        {
          "hubTracking.customerHubId": { $exists: true, $ne: null }
        }
    ...
    // Additional filter to ensure no delivered orders slip through (safety check)
    const filteredOrders = orders.filter(order => order.orderStatus !== 'delivered');
    
    console.log(`✅ Found ${filteredOrders.length} customer hub orders (excluding delivered)`);
...
router.get("/dashboard-stats", async (req, res) => {
  try {
    ...
    const [
      ...
      // Orders at customer hubs (excluding delivered)
      Order.countDocuments({
        $and: [
          { orderStatus: { $in: ['at_customer_hub', 'out_for_delivery', 'ready_for_pickup'] } },
