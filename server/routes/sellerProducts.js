...
router.post("/", verify, verifySeller, upload.single("image"), handleMulterError, async (req, res) => {
  try {
    ...
    const { title, mainCategory, subCategory, category, stock, variants, manufacturingDate, expiryDate } = req.body;
...
router.put("/:id", verify, verifySeller, upload.single("image"), handleMulterError, async (req, res) => {
  try {
    ...
    if (req.body.manufacturingDate) {
      ...
      updates.manufacturingDate = mfgDate;
    }
    ...
    if (req.body.expiryDate) {
      ...
      const mfgDate = updates.manufacturingDate || existingProduct.manufacturingDate;
      if (mfgDate && expDate <= new Date(mfgDate)) {
        return res.status(400).json({ error: "Expiry date must be after manufacturing date" });
      }
