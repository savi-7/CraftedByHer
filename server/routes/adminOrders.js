...
router.get("/hub-orders/pending", verify, verifyAdmin, async (req, res) => {
  try {
    ...
    // Format orders for admin display and calculate customer hub
    const formattedOrders = await Promise.all(pendingOrders.map(async (order) => {
      // Calculate customer hub based on customer address
      let customerHub = null;
      if (order.buyerDetails?.address) {
        const customerDistrict = extractDistrict(order.buyerDetails.address);
        console.log(`🏙️  Customer district for order ${order.orderNumber}: ${customerDistrict}`);
        
        // Find customer hub in the same district
        ...
        if (customerHub) {
          ...
          console.log(`⚠️  No customer hub found for district: ${customerDistrict}`);
        }
      }
      ...
      return {
        ...
        // Include full hubTracking object
        hubTracking: {
          sellerHubId: order.hubTracking?.sellerHubId || null,
          sellerHubName: order.hubTracking?.sellerHubName || null,
          sellerHubDistrict: order.hubTracking?.sellerHubDistrict || null,
          customerHubId: order.hubTracking?.customerHubId || (customerHub ? customerHub.hubId : null),
          customerHubName: order.hubTracking?.customerHubName || (customerHub ? customerHub.name : null),
          customerHubDistrict: order.hubTracking?.customerHubDistrict || (customerHub ? customerHub.district : null),
          arrivedAtSellerHub: order.hubTracking?.arrivedAtSellerHub || null,
          approvedByAdmin: order.hubTracking?.approvedByAdmin || false
        },
        // Keep hubInfo for backward compatibility
        hubInfo: {
