...
const { createAdminHubNotification, createHubManagerNotification, createOrderDispatchedNotification, createCustomerHubArrivalNotification } = require("../utils/notificationService");
...
router.post("/:orderId/move-to-seller-hub", verify, async (req, res) => {
  try {
    ...
    // Create hub manager notification for approval (instead of admin)
    try {
      await createHubManagerNotification(order, {
        name: sellerHub.name,
        district: sellerHub.district,
        hubId: sellerHub.hubId
      });
      console.log(`✅ Hub manager notification created for order ${order.orderNumber}`);
    } catch (notificationError) {
      console.error("Failed to create hub manager notification:", notificationError);
      // Don't fail the main operation if notification fails
    }
    
    // Send email notification to hub manager
