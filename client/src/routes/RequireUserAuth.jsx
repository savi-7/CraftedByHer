...
export default function RequireUserAuth({ children }) {
  ...
  // Exception: allow sellers to access cake preorder flow and payment (they can buy cakes too)
  const path = location.pathname || '';
  const isCakePreorderFlow = path.startsWith('/cakes/preorder');
  const isPaymentFlow = path === '/payment-selection' || path === '/payment' || path.startsWith('/order-confirmation');
  if (userRole === 'seller' && !isCakePreorderFlow && !isPaymentFlow) {
