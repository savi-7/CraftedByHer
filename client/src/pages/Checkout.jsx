...
export default function Checkout() {
  ...
  const validateForm = () => {
    // Check if hub availability has been verified
    if (!canProceed) {
    ...
    // If using a saved address, no need to validate form fields
    if (usingSavedAddress) {
      console.log('✅ Using saved address - no validation needed');
      return true;
    }

    // If showing new address form, validate all required fields
    if (showNewAddressForm) {
      const errors = {};
      
      // Validate name
      if (!buyerDetails.name || buyerDetails.name.trim().length < 2) {
        errors.name = "Please enter your full name (at least 2 characters)";
      }
      
      // Validate phone
      if (!buyerDetails.phone || !/^[6-9]\d{9}$/.test(buyerDetails.phone)) {
        errors.phone = "Please enter a valid 10-digit phone number starting with 6-9";
      }
      
      // Validate street address
      if (!buyerDetails.address.street || buyerDetails.address.street.trim().length < 5) {
        errors["address.street"] = "Please enter a complete street address (at least 5 characters)";
      }
      
      // Validate city
      if (!buyerDetails.address.city || buyerDetails.address.city.trim().length < 2) {
        errors["address.city"] = "Please enter your city";
      }
      
      // Validate state
      if (!buyerDetails.address.state || buyerDetails.address.state.trim().length < 2) {
        errors["address.state"] = "Please enter your state";
      }
      
      // Pincode is already validated through hub check
      ...
      setValidationErrors({});
      console.log('✅ All address fields validated');
    }
  ...
  const handleProceedToPayment = async () => {
    ...
    // Validate form (includes hub availability and address fields if new address)
    if (!validateForm()) {
    ...
    try {
      ...
      // If using a saved address, get the full details
      if (usingSavedAddress && selectedAddressId) {
        const selectedAddr = savedAddresses.find(addr => addr._id === selectedAddressId);
        if (selectedAddr) {
          finalBuyerDetails = {
            name: selectedAddr.name || cleanDisplayName(user.displayName) || "",
            email: user.email || "",
      ...
      if (showNewAddressForm && saveThisAddress) {
        try {
          ...
          if (saveResp.ok) {
            ...
            if (updatedAddresses.ok) {
              const updatedData = await updatedAddresses.json();
              const addresses = Array.isArray(updatedData.addresses) ? updatedData.addresses : [];
              setSavedAddresses(addresses);
              
              // If this was set as default, select it
              if (setAsDefault && addresses.length > 0) {
                const newAddr = addresses.find(addr => 
                  addr.address.pincode === buyerDetails.address.pincode &&
                  addr.address.street === buyerDetails.address.street
                );
                if (newAddr) {
                  setSelectedAddressId(newAddr._id);
                  // Update buyerDetails to use the saved address
                  finalBuyerDetails = {
                    name: newAddr.name || buyerDetails.name,
                    email: user.email || "",
                    phone: newAddr.phone || buyerDetails.phone,
            ...
            console.error('❌ Failed to save address:', saveResp.status, errorText);
            toast.warning("Address not saved, but continuing with order");
          }
        } catch (saveError) {
      ...
      console.log('Buyer details:', finalBuyerDetails);
      
      // Navigate to payment selection page with buyer details
      navigate("/payment-selection", {
        state: {
          cartItems,
          deliveryPincode: finalBuyerDetails.address.pincode || pincodeInput,
          deliveryCity: deliveryCity,
          buyerDetails: finalBuyerDetails, // Pass buyer details so PaymentSelection can use them
          fromEventOrder: location.state?.fromEventOrder,
          fromBulkOrder: location.state?.fromBulkOrder,
        }
      });
          ...
          <div style={{
            ...
            <div style={{ marginBottom: "20px", maxHeight: "300px", overflowY: "auto" }}>
              {cartItems.length === 0 ? (
                ...
                // Try to get image from multiple sources (Order Planner structure first)
                if (item.product && typeof item.product === 'object') {
                  // Order Planner structure: product is an object
                  rawImagePath = item.product.image || item.product.img;
                ...
                // Get product title (check Order Planner structure first)
                ...
                // Generate unique key - use product ID, variant ID/weight, and index to ensure uniqueness
                const productId = item.product?._id 
                  || (typeof item.productId === 'object' ? item.productId?._id : item.productId)
                  || `product-${index}`;
                const variantId = item.variant?._id || item.variant?.weight || `variant-${index}`;
                const uniqueKey = `${productId}-${variantId}-${index}`;
