                      ...
                      <div style={{ 
                        ...
                        <div style={{ fontSize: "13px", color: "var(--text-muted, #7b6457)" }}>
                          ...
                          <span style={{ fontWeight: 600, color: "var(--text, #3f2d23)" }}>
                            {order.hubTracking?.sellerHubName || order.hubInfo?.hubName || "N/A"}
                          </span>
                          {(order.hubTracking?.sellerHubDistrict || order.hubInfo?.hubDistrict) && (
                            <span style={{ fontSize: "12px", color: "var(--text-muted, #7b6457)", marginLeft: "4px" }}>
                              ({order.hubTracking?.sellerHubDistrict || order.hubInfo?.hubDistrict})
                            </span>
                          )}
                        </div>
                        <div style={{ fontSize: "13px", color: "var(--text-muted, #7b6457)" }}>
                          To Hub:&nbsp;
                          <span style={{ fontWeight: 600, color: "var(--brand, #8b5e34)" }}>
                            {order.hubTracking?.customerHubName || "Not Assigned"}
                          </span>
                          {order.hubTracking?.customerHubDistrict && (
                            <span style={{ fontSize: "12px", color: "var(--text-muted, #7b6457)", marginLeft: "4px" }}>
                              ({order.hubTracking.customerHubDistrict})
                            </span>
                          )}
                      ...
                      {order.items && order.items.length > 0 && (
                        <div style={{ 
                          ...
                          <div style={{ 
                            ...
                            Product Details ({order.items.length} {order.items.length > 1 ? "items" : "item"})
                          </div>
                          <div style={{ 
                            display: "flex", 
                            flexDirection: "column", 
                            ...
                            {order.items.map((item, itemIndex) => {
                              const itemTitle = item.title || (item.productId && item.productId.title) || "Product";
                              const itemImage = item.image || (item.productId && item.productId.image);
                              const itemPrice = item.variant?.price || 0;
                              const itemQuantity = item.quantity || 1;
                              const itemVariant = item.variant?.weight || "";
                              ...
                              return (
                                ...
                                >
                                  <div style={{
                                    ...
                                    {itemImage ? (
                                      <img
                                        src={`${API_BASE}/uploads/${itemImage}`}
                                        alt={itemTitle}
                                        style={{ width: "100%", height: "100%", objectFit: "cover" }}
                                      />
                                    ) : (
                                      <span style={{ fontSize: "18px", color: "var(--text-muted, #7b6457)" }}>📦</span>
                                    )}
                                  </div>
                                  <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{ 
                                      ...
                                      Qty: {itemQuantity} {itemVariant && `• ${itemVariant}`}
                                    </div>
                                    <div style={{ 
                                      fontSize: "12px", 
                                      fontWeight: 600, 
                                      ...
                                      ₹{itemPrice.toLocaleString("en-IN")} × {itemQuantity} = ₹{itemTotal.toLocaleString("en-IN")}
                                    </div>
                                  </div>
                                </div>
                              );
