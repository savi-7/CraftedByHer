...
export default function HubManagerDashboard() {
  ...
  const fetchDashboardData = async (token) => {
    ...
    try {
      ...
      const statsRes = await fetch(`${API_BASE}/api/hub-managers/dashboard/stats`, {
      ...
      const hubsRes = await fetch(`${API_BASE}/api/hubs/all-with-stats`).catch(err => {
      ...
      try {
        notificationsRes = await fetch(`${API_BASE}/api/hub-notifications?managerId=${managerId}`);
      ...
      const ordersRes = await fetch(`${API_BASE}/api/hub-managers/orders/hub`, {
        headers: { Authorization: `Bearer ${token}` }
      }).catch(err => {
      ...
      // Load seller hub orders + pending approvals (needed for "seller hub field" + approve button)
      const [sellerHubRes, approvalsRes] = await Promise.all([
        fetch(`${API_BASE}/api/hub-managers/orders/seller-hub`, { headers: { Authorization: `Bearer ${token}` } })
          .catch(() => ({ ok: false, json: () => Promise.resolve({ success: false, orders: [] }) })),
        fetch(`${API_BASE}/api/hub-managers/orders/pending-approvals`, { headers: { Authorization: `Bearer ${token}` } })
          .catch(() => ({ ok: false, json: () => Promise.resolve({ success: false, orders: [] }) })),
      ]);
      ...
      let sellerHubData = { success: false, orders: [] };
      if (sellerHubRes.ok) {
        try {
          sellerHubData = await sellerHubRes.json();
        } catch (e) {
          console.error("Error parsing seller hub orders response:", e);
        }
      }

      let approvalsData = { success: false, orders: [] };
      if (approvalsRes.ok) {
        try {
          approvalsData = await approvalsRes.json();
        } catch (e) {
          console.error("Error parsing approvals response:", e);
        }
      }
      ...
      if (sellerHubData.success) {
        setSellerHubOrders(sellerHubData.orders || []);
      } else {
        setSellerHubOrders([]);
      }

      if (approvalsData.success) {
        setApprovalOrders(approvalsData.orders || []);
      } else {
        setApprovalOrders([]);
      }
  ...
  const debugNotifications = async () => {
    ...
    console.log("API URL:", `${API_BASE}/api/hub-notifications?managerId=${managerId}`);
    
    try {
      const response = await fetch(`${API_BASE}/api/hub-notifications?managerId=${managerId}`);
  ...
  const handleNotificationClick = async (notification) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/hub-managers/notifications/${notification._id}/details`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
  ...
  const markAllAsRead = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/hub-managers/notifications/read-all`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` }
        }
  ...
  const approveOrder = async (orderMongoId) => {
    const token = localStorage.getItem("hubManagerToken");
    if (!token) {
    ...
    setApprovingOrder(orderMongoId);
    try {
      const res = await fetch(`${API_BASE}/api/hub-managers/orders/${orderMongoId}/approve-and-dispatch`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
        ...
        <div className="bk-tab-content">
          ...
          {activeTab === "orders" && (
            <div>
              ...
              {sellerHubOrders.length > 0 && (
                <div className="mb-8">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-bold" style={{ color: "var(--brand)" }}>Seller Hub Orders</h3>
                    <span className="text-sm" style={{ color: "var(--text-muted)" }}>
                      {sellerHubOrders.length} order(s) at seller hub
                    </span>
                  </div>
                  <div className="space-y-3">
                    {sellerHubOrders.map((o) => (
                      <div key={o._id} className="bk-order-card">
                        <div className="flex items-center justify-between mb-2">
                          <div>
                            ...
                            <p className="text-sm" style={{ color: "var(--text-muted)" }}>
                              {new Date(o.hubTracking?.arrivedAtSellerHub || o.createdAt).toLocaleString()}
                            </p>
                            {o.isCakePreorder && (
                              <p className="text-sm mt-2" style={{ color: "var(--text-muted)" }}>
                                🎂 Cake preorder has reached your hub. Approve to send OTP to buyer.
                              </p>
                            )}
                          </div>
                          <button
                            onClick={() => approveOrder(o._id)}
                            disabled={approvingOrder === o._id}
                            className="bk-btn-primary"
                            style={{
                              opacity: approvingOrder === o._id ? 0.7 : 1,
                              cursor: approvingOrder === o._id ? "not-allowed" : "pointer",
                              minWidth: 140
                            }}
                          >
                            {approvingOrder === o._id ? "Approving..." : "Approve"}
                          </button>
                        </div>
              ...
              {orders.length === 0 && sellerHubOrders.length === 0 ? (
                <div className="bk-empty-state">
      ...
      {selectedNotification && (
        <div className="bk-modal-overlay" onClick={() => setSelectedNotification(null)}>
          <div className="bk-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '700px' }}>
            ...
            {/* Actions */}
            <div className="bk-modal-footer" style={{ padding: "16px 24px", borderTop: "1px solid var(--border)" }}>
              ...
              (selectedNotification.actionType === "approve_hub_delivery" ||
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    onClick={() => approveOrder(selectedNotification.orderId)}
                    disabled={approvingOrder === selectedNotification.orderId}
                    style={{ opacity: approvingOrder === selectedNotification.orderId ? 0.7 : 1 }}
                  >
                    {approvingOrder === selectedNotification.orderId ? "Approving..." : "Approve"}
                  </button>
                  <button className="bk-btn-secondary" onClick={() => setSelectedNotification(null)}>
                    Close
                  </button>
                </div>
              ) : (
                <button className="bk-btn-primary w-full" onClick={() => setSelectedNotification(null)}>
                  Close
                </button>
              )}
            </div>
