...
export default function Login() {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      // Handle Firebase authentication errors FIRST (these happen before backend calls)
      if (err?.code) {
        ...
        const errorMsg = firebaseErrorMessages[err.code] || `Authentication failed: ${err.message || "Unknown error"}`;
        toast.error(errorMsg, { className: "custom-toast", autoClose: 4000 });
        return;
      }
      
      // Handle backend/API errors (shouldn't reach here if Firebase auth failed)
      let errorMessage = "";
      ...
      if (errorMessage.includes("Seller application not approved")) {
        ...
        toast.error(errorMessage, { className: "custom-toast", autoClose: 3000 });
      }
    } finally {
      setLoadingEmail(false);
    }
