...
export default function SellerDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.manufacturingDate || form.manufacturingDate.trim() === "") {
    ...
    if (!form.expiryDate || form.expiryDate.trim() === "") {
      ...
      const mfgDate = form.manufacturingDate ? new Date(form.manufacturingDate) : null;
      const today = new Date();
      today.setHours(0, 0, 0, 0);
    ...
    try {
      ...
      if (response.ok) {
        ...
        setForm({ title: "", mainCategory: "", subCategory: "", stock: "", price: "", variants: [{ weight: "", price: "" }], image: null, manufacturingDate: "", expiryDate: "" });
              ...
              <form onSubmit={handleSubmit} style={{ display: "grid", gap: "16px" }}>
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      const value = e.target.value;
                      setForm({ ...form, manufacturingDate: value });
                      // Validate manufacturing date
                      if (!value) {
                        setErrors((prev) => ({ ...prev, manufacturingDate: "Manufacturing date is required" }));
                      } else {
                        const mfgDate = new Date(value);
                        const today = new Date();
                        today.setHours(0, 0, 0, 0);
                        if (mfgDate > today) {
                          setErrors((prev) => ({ ...prev, manufacturingDate: "Manufacturing date cannot be in the future" }));
                        } else {
                    ...
                    style={{
                      ...inputStyle,
                      borderColor: errors.manufacturingDate ? "#d9534f" : inputStyle.borderColor
                    }}
                    required
                  />
                  <p style={{ fontSize: "12px", color: "#666", marginTop: "5px" }}>
                    Enter the date when the product was manufactured
                  </p>
                  {errors.manufacturingDate && (
                    <div style={{ color: "#d9534f", fontSize: "12px", marginTop: "6px" }}>{errors.manufacturingDate}</div>
                  )}
                </div>
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      if (!value) {
                        ...
                        if (mfgDate && expiryDate <= mfgDate) {
                          setErrors((prev) => ({ ...prev, expiryDate: "Expiry date must be after manufacturing date" }));
                        } else if (expiryDate <= today) {
                          setErrors((prev) => ({ ...prev, expiryDate: "Expiry date must be in the future" }));
                        } else {
                    ...
                    style={{
                      ...inputStyle,
                      borderColor: errors.expiryDate ? "#d9534f" : inputStyle.borderColor
                    }}
                    required
                    min={form.manufacturingDate || undefined}
                  />
                  <p style={{ fontSize: "12px", color: "#666", marginTop: "5px" }}>
                    Enter the expiry date of the product. Product becomes non-buyable 10 days before expiry.
                  </p>
                  {errors.expiryDate && (
                    <div style={{ color: "#d9534f", fontSize: "12px", marginTop: "6px" }}>{errors.expiryDate}</div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {editingId && (
                    <button
                      type="button"
                      onClick={() => { setEditingId(null); setForm({ title: "", mainCategory: "", subCategory: "", stock: "", price: "", variants: [{ weight: "", price: "" }], image: null, manufacturingDate: "", expiryDate: "" }); setSelectedFile(null); setCurrentImageUrl(null); }}
    ...
    No Image
                      ...
                      {p.manufacturingDate && (
                        <p style={{ fontSize: "12px", color: "#666", margin: "3px 0" }}>
                          📅 Mfg: {new Date(p.manufacturingDate).toLocaleDateString()}
                        </p>
                      )}
                      {p.expiryDate && (
                        <p style={{ fontSize: "12px", color: "#666", margin: "3px 0" }}>
                      ...
                      {p.expiryDate && (() => {
                        ...
                        const isBuyable = today <= cutoffDate;
                        return !isBuyable ? (
                          <div style={{
                            marginTop: "8px",
                            padding: "4px 8px",
                            ...
                            ⚠️ Not Buyable (Within 10 days of expiry)
                          </div>
                        ) : null;
                      })()}
                ...
                {orders.map((order) => (
                  ...
                  onMouseLeave={(e) => {
                      ...
                      <div>
                        <h3 style={{ margin: "0 0 8px 0", color: "#5c4033", fontSize: "18px" }}>
                          ...
                          {order.isCakePreorder && (
                            <span style={{ marginLeft: "8px", padding: "2px 8px", background: "#fff3cd", color: "#856404", borderRadius: "6px", fontSize: "11px", fontWeight: "600" }}>
                              Cake preorder
                            </span>
                          )}
                        </h3>
                    ...
                    <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
                      ...
                      {/* Cake preorder: Start production → Complete production → Move to Hub */}
                      {order.isCakePreorder && (order.orderStatus === 'pending' || order.orderStatus === 'confirmed') && !order.hubTracking?.sellerHubId && (
                          ...
                          {!order.productionStartedAt && (
                            <button
                              onClick={async () => {
                                try {
                                  const token = await auth.currentUser.getIdToken();
                                  const r = await fetch(`${API_BASE}/api/seller/orders/${order._id}/start-production`, {
                                    method: "PATCH",
                              ...
                              style={{ padding: "8px 14px", background: "#17a2b8", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "12px", fontWeight: "600" }}
                            >
                              Start production
                            </button>
                          )}
                          {order.productionStartedAt && !order.productionCompletedAt && (
                            <button
                              onClick={async () => {
                                try {
                                  const token = await auth.currentUser.getIdToken();
                                  const r = await fetch(`${API_BASE}/api/seller/orders/${order._id}/complete-production`, {
                                    method: "PATCH",
                                    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                                  });
                                  if (r.ok) {
                              ...
                              style={{ padding: "8px 14px", background: "#fd7e14", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "12px", fontWeight: "600" }}
                            >
                              Complete production
                            </button>
                          )}
                          {order.productionCompletedAt && (
                            <button
                              onClick={async () => {
                                try {
                                  const token = await auth.currentUser.getIdToken();
                                  const hubResponse = await fetch(`${API_BASE}/api/seller/orders/nearest-hub`, { headers: { Authorization: `Bearer ${token}` } });
                                  if (!hubResponse.ok) {
                                    const err = await hubResponse.json();
                                    if (err.sellerLocationMissing) { toast.error("Please set your location first"); setActiveSection("settings"); return; }
                                    throw new Error(err.error || "Failed to find hub");
                                  }
                                  const hubData = await hubResponse.json();
                                  if (!hubData.nearestHub) { toast.error("No active hubs in your area"); return; }
                                  const moveResponse = await fetch(`${API_BASE}/api/seller/orders/${order._id}/move-to-hub`, {
                                    method: "PATCH",
                                    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                                    body: JSON.stringify({ hubId: hubData.nearestHub.hubId }),
                                  });
                                  if (moveResponse.ok) {
                                    toast.success(`Order moved to ${hubData.nearestHub.name}`);
                                    await fetchOrders(auth.currentUser);
                                  } else {
                                    const d = await moveResponse.json();
                                    toast.error(d.error || "Failed to move to hub");
                                  }
                                } catch (error) {
                                  toast.error("Failed to move to hub");
                                }
                              }}
                              style={{ padding: "8px 14px", background: "#28a745", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "12px", fontWeight: "600" }}
                            >
                              Move to Hub
                            </button>
                          )}
                      ...
                      {/* Regular order or post-hub: Move to Hub Button */}
                      {!order.isCakePreorder && (order.orderStatus === 'pending' || order.orderStatus === 'confirmed') && !order.hubTracking?.sellerHubId ? (
                        <button
                          onClick={async () => {
                            try {
          ...
          <div style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
              <div>
                ...
                <p><strong>Order ID:</strong> #{selectedOrder.orderNumber || selectedOrder.id || selectedOrder._id}
                {selectedOrder.isCakePreorder && (
                  <span style={{ marginLeft: "8px", padding: "2px 8px", background: "#fff3cd", color: "#856404", borderRadius: "6px", fontSize: "12px", fontWeight: "600" }}>
                    Cake preorder
                  </span>
                )}
                </p>
            ...
            {/* Cake preorder customization details */}
            {selectedOrder.isCakePreorder && selectedOrder.cakePreorderDetails && (
              <div style={{ marginBottom: "20px", padding: "16px", background: "#fff8e6", border: "1px solid #f3d19c", borderRadius: "10px" }}>
                <h3 style={{ marginBottom: "12px", color: "#5c4033" }}>🎂 Cake customization</h3>
                <p style={{ margin: "4px 0", fontSize: "14px" }}><strong>Size:</strong> {selectedOrder.cakePreorderDetails.size || "-"}</p>
                <p style={{ margin: "4px 0", fontSize: "14px" }}><strong>Message on cake:</strong> {selectedOrder.cakePreorderDetails.messageOnCake || "None"}</p>
                <p style={{ margin: "4px 0", fontSize: "14px" }}><strong>Type:</strong> {selectedOrder.cakePreorderDetails.eggOrEggless || "eggless"}</p>
                {selectedOrder.cakePreorderDetails.extraToppings?.length > 0 && (
                  <p style={{ margin: "4px 0", fontSize: "14px" }}><strong>Extra toppings:</strong> {selectedOrder.cakePreorderDetails.extraToppings.join(", ")}</p>
                )}
                <p style={{ margin: "4px 0", fontSize: "14px" }}><strong>Delivery date:</strong> {selectedOrder.cakePreorderDetails.deliveryDate ? new Date(selectedOrder.cakePreorderDetails.deliveryDate).toLocaleDateString() : "-"}</p>
              </div>
            )}
            ...
            <div style={{ marginTop: "20px", display: "flex", gap: "10px", justifyContent: "space-between" }}>
              <div>
                {/* Cake preorder: Start → Complete → Move to Hub */}
                {selectedOrder.isCakePreorder && (selectedOrder.orderStatus === 'pending' || selectedOrder.orderStatus === 'confirmed') && !selectedOrder.hubTracking?.sellerHubId && (
                    ...
                    {!selectedOrder.productionStartedAt && (
                      <button
                        onClick={async () => {
                          try {
                            const token = await auth.currentUser.getIdToken();
                            const r = await fetch(`${API_BASE}/api/seller/orders/${selectedOrder._id}/start-production`, {
                              method: "PATCH",
                              headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                            });
                            if (r.ok) { toast.success("Production started"); closeOrderModal(); await fetchOrders(auth.currentUser); }
                            else { const d = await r.json(); toast.error(d.error || "Failed"); }
                          } catch (e) { toast.error("Failed"); }
                        }}
                        style={{ padding: "10px 18px", background: "#17a2b8", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "14px", fontWeight: "600" }}
                      >
                        Start production
                      </button>
                    )}
                    {selectedOrder.productionStartedAt && !selectedOrder.productionCompletedAt && (
                      <button
                        onClick={async () => {
                          try {
                            const token = await auth.currentUser.getIdToken();
                            const r = await fetch(`${API_BASE}/api/seller/orders/${selectedOrder._id}/complete-production`, {
                              method: "PATCH",
                              headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                            });
                            if (r.ok) { toast.success("Production completed"); closeOrderModal(); await fetchOrders(auth.currentUser); }
                            else { const d = await r.json(); toast.error(d.error || "Failed"); }
                          } catch (e) { toast.error("Failed"); }
                        }}
                        style={{ padding: "10px 18px", background: "#fd7e14", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "14px", fontWeight: "600" }}
                      >
                        Complete production
                      </button>
                    )}
                    {selectedOrder.productionCompletedAt && (
                      <button
                        onClick={async () => {
                          try {
                            const token = await auth.currentUser.getIdToken();
                            ...
                            if (!hubResponse.ok) {
                              const err = await hubResponse.json();
                              if (err.sellerLocationMissing) { toast.error("Set your location first"); closeOrderModal(); setActiveSection("settings"); return; }
                              throw new Error(err.error);
                            }
                            const hubData = await hubResponse.json();
                            if (!hubData.nearestHub) { toast.error("No hubs in your area"); return; }
                            if (!confirm(`Move this cake preorder to ${hubData.nearestHub.name}?`)) return;
                            ...
                            if (moveResponse.ok) {
                              toast.success(`Moved to ${hubData.nearestHub.name}`);
                              closeOrderModal();
                              await fetchOrders(auth.currentUser);
                            } else {
                              const d = await moveResponse.json();
                          ...
                          } catch (e) { toast.error("Failed to move to hub"); }
                        }}
                        style={{ padding: "10px 18px", background: "#28a745", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "14px", fontWeight: "600" }}
                      >
                        Move to Hub
                      </button>
                    )}
                ...
                {/* Move to Hub Button (regular orders) */}
                {!selectedOrder.isCakePreorder && (selectedOrder.orderStatus === 'pending' || selectedOrder.orderStatus === 'confirmed') && !selectedOrder.hubTracking?.sellerHubId && (
                  <button
                    onClick={async () => {
                      try {
